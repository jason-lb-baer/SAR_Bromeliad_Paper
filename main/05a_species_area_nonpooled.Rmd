...
ggsave("/Users/jasonbaer/Downloads/SAR paper/all_sar_nonpool_plots.pdf", width=16, height=7.5)
ggsave("/Users/jasonbaer/Downloads/SAR paper/all_sar_nonpool_plots.svg", width=16, height=7.5)

# Save using svglite
svglite::svglite(
  filename = "/Users/jasonbaer/Downloads/SAR paper/all_sar_nonpool_plots.svg",
  width = 16,
  height = 7.5
)
print(all_sar_plots)
...
Jason duplicate of above & modified to output svg and standardize axes DONE
```{r}

# Standardize Fungal Plots: ylim(0, 5)
fung_det_plot <- lapply(fung_det_plot, function(p) p + ylim(0, 5))
fung_water_plot <- lapply(fung_water_plot, function(p) p + ylim(0, 5))
fung_inv_plot <- lapply(fung_inv_plot, function(p) p + ylim(0, 5))

# Standardize Bacterial Plots: ylim(4, 7)
bact_det_plot <- lapply(bact_det_plot, function(p) p + ylim(4, 7))
bact_water_plot <- lapply(bact_water_plot, function(p) p + ylim(4, 7))
bact_inv_plot <- lapply(bact_inv_plot, function(p) p + ylim(4, 7))

library(patchwork)
library(svglite)

all_sar_plots <- 
  fung_det_plot[[2]] + fung_water_plot[[1]] + 
  fung_inv_plot[[2]] + fung_inv_plot[[1]] +
  bact_det_plot[[2]] + bact_water_plot[[1]] +
  bact_inv_plot[[2]] + bact_inv_plot[[1]] +
  plot_layout(nrow = 2)
...
# Convert p-values to numeric (you already did this)
full_full_tbl$bootstrap_ttest_pval <- as.numeric(full_full_tbl$bootstrap_ttest_pval)

# Adjust using Benjamini-Hochberg
full_full_tbl$bootstrap_ttest_padj <- p.adjust(full_full_tbl$bootstrap_ttest_pval, method = "BH")
...
# Set seed (for any reproducible random operations — not necessary for p.adjust itself)
set.seed(202415)

# Adjust p-values using BH method
full_btwn$adjusted_pval <- p.adjust(full_btwn$pval, method = "BH")

# Proceed with renaming and formatting
...
# Save the output
write.csv(full_btwn_renamed, "/Users/jasonbaer/Downloads/SAR paper/outputs/for_manuscript/bootslope_ttest_btwn_loci_noair_morestats.csv", row.names = FALSE)
...
fung_air <- readRDS("/Users/jasonbaer/Downloads/SAR paper/intermediates/truly_nonpooled_fung_air_data.rds")
bact_air <- readRDS("/Users/jasonbaer/Downloads/SAR paper/intermediates/truly_nonpooled_bact_air_data.rds")
```
...
ggsave("/Users/jasonbaer/Downloads/SAR paper/z_score_plot2.pdf", width=8, height=5.5)
ggsave("/Users/jasonbaer/Downloads/SAR paper/z_score_plot2.png", width=8, height=5.5)
...
ggsave("/Users/jasonbaer/Downloads/SAR paper/z_score_plot3.pdf", width=8, height=5.5)
ggsave("/Users/jasonbaer/Downloads/SAR paper/z_score_plot3.png", width=8, height=5.5)
```


Jason duplicate and modify to remove non-significant bars, save as svg DONE
...
# Subset to only the significant cases
slope_df_filtered <- slope_df %>%
  dplyr::filter(
    (locus == "Fungi" & compartment %in% c("Detritus", "Water")) |
    (locus == "Bacteria" & compartment == "Water")
  )

# Reorder factors to maintain layout
slope_df_filtered$compartment <- factor(slope_df_filtered$compartment, 
                                        levels = c("Detritus", "Water", "Invertebrates (d)", "Invertebrates (w)"))
slope_df_filtered$locus <- factor(slope_df_filtered$locus, levels = c("Fungi", "Bacteria"))

# Create the bar plot
library(ggplot2)
library(metR)  # If needed for met.brewer
library(svglite)

z_plot <- ggplot(slope_df_filtered, aes(
  fill = compartment,
  y = slope,
  x = locus
)) + 
...
svglite::svglite("/Users/jasonbaer/Downloads/SAR paper/z_score_plot2.svg", width = 8, height = 5.5)
print(z_plot)
dev.off()
